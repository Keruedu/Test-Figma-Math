        ...
        <div className="relative z-10 container mx-auto px-4 py-8 max-w-4xl">
          <div className="text-center mb-8 animate-fade-slide">
            ...
            <div className={`inline-block ${cardBgClass} backdrop-blur-sm px-8 py-4 rounded-2xl border-2 border-[#6E55FB] ${theme === 'dark' ? '' : 'shadow-xl'}`}>
              <p className={`mb-2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>Mã phòng:</p>
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 max-h-64 overflow-y-auto">
                {players.map((player, index) => (
                  <div
                    key={player.id}
                    className={`${inputBgClass} rounded-xl p-3 flex items-center gap-3 animate-fade-slide ${theme === 'dark' ? 'hover:bg-white/20' : 'hover:bg-gray-200'} transition-all`}
                    style={{ animationDelay: `${index * 0.1}s` }}
                  >
                    <span className="text-3xl">{player.avatar}</span>
                    <span className={`${textClass} font-medium truncate`}>{player.name}</span>
                  </div>
                ))}
              </div>
            </div>
