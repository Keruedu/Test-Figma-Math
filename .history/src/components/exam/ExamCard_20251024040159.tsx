...
export default function ExamCard({ exam, onSelectExam, onRetakeExam, onViewResult }: ExamCardProps) {
  ...
  // Tính điểm thang 10: (số câu đúng / tổng số câu) × 10
  const score10Scale = exam.score !== undefined 
    ? ((exam.score / (exam.questionCount * 2)) * 10).toFixed(1)
    : null
  ...
  return (
    <div className={`border-2 rounded-3xl p-8 transition-all hover:scale-[1.02] relative overflow-hidden group flex flex-col ${
      ...
      <div className='flex flex-row gap-2 md:gap-6'>
        <div className="w-20 h-20 aspect-square bg-gradient-to-br from-[#6E55FB] to-purple-600 rounded-3xl flex items-center justify-center mb-5 group-hover:scale-110 transition-transform shadow-lg">
        ...
        <div className="flex flex-col gap-2 mb-5">
            <h3 className="text-2xl font-extrabold text-gray-900 leading-tight text-left">{exam.name}</h3>
            <span className="bg-purple-100 text-[#6E55FB] px-2 py-1.5 rounded-full text-sm font-bold whitespace-nowrap w-fit">
      ...
      <div className="space-y-3 mb-6 flex-1">
        ...
        {/* Hàng 2: Số câu + Điểm thang 10 (nếu có) */}
        <div className="flex items-center justify-between gap-4">
          {/* Số câu */}
          <div className="flex items-center gap-2 flex-1">
            <div className="w-9 h-9 rounded-xl bg-purple-100 flex items-center justify-center flex-shrink-0">
          ...
          {/* Điểm thang 10 - Nổi bật với màu xanh (nếu có) */}
          {exam.completed && score10Scale ? (
            <div className="flex items-center gap-2 flex-1">
