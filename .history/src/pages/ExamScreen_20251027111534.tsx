...
export default function ExamScreen({ initialTab = 'system' }: ExamScreenProps = {}) {
  const [currentScreen, setCurrentScreen] = useState<ScreenType>('list')
  const [selectedExam, setSelectedExam] = useState<Exam | null>(null)
  const [selectedTab, setSelectedTab] = useState<'system' | 'ai'>(initialTab)
  const [filteredExams, setFilteredExams] = useState<Exam[]>(mockExams.filter(e => e.type === initialTab))
  ...
  useEffect(() => {
    handleFilter()
  }, [selectedTab, filterChapter, filterLesson, filterStatus, filterExamType])
  ...
  const handleCreateAIExam = () => {
    // TODO: Implement AI exam creation logic
    alert('Chức năng tạo bài kiểm tra với AI sẽ được phát triển!')
  }
  ...
  if (currentScreen === 'list') {
    return (
      <ExamListScreen
        ...
        onCreateAIExam={handleCreateAIExam}
      />
    )
  }
