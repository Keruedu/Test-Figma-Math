...
const mockExams: Exam[] = [
  ...
  { id: 5, name: 'Bài luyện tập nhanh (AI)', type: 'ai', subject: 'Toán', chapter: 'Chương 1', lesson: 'Bài 1', duration: 20, questionCount: 15, grade: 'Lớp 3', examType: 'practice', completed: false },
  { id: 6, name: 'Tổng hợp kiến thức (AI)', type: 'ai', subject: 'Toán', chapter: 'Chương 1', lesson: 'Tổng hợp', duration: 30, questionCount: 20, grade: 'Lớp 3', examType: 'review', completed: false },
  { id: 7, name: 'Ôn tập Số học (AI)', type: 'ai', subject: 'Toán', chapter: 'Chương 2', lesson: 'Bài 3', duration: 25, questionCount: 12, grade: 'Lớp 3', examType: 'practice', completed: false }
]
...
export const ExamScreen = () => {
        ...
        setTimeRemaining((prev) => {
          if (prev <= 1) {
            ...
            if (selectedExam) {
              ...
              setExamResults(prevResults => ({ ...prevResults, [selectedExam.id]: result }))
              
              const examIndex = mockExams.findIndex(e => e.id === selectedExam.id)
              if (examIndex !== -1) {
                mockExams[examIndex].completed = true
                mockExams[examIndex].score = result.score
                mockExams[examIndex].completedAt = result.completedAt
              }
  ...
  const handleFilter = () => {
    ...
    if (filterStatus === 'completed') filtered = filtered.filter(e => e.completed === true)
    if (filterStatus === 'notCompleted') filtered = filtered.filter(e => e.completed !== true)
    if (filterExamType) filtered = filtered.filter(e => e.examType === filterExamType)
    
    // Sắp xếp: chưa làm lên trên, đã làm xuống dưới
    filtered.sort((a, b) => {
  ...
  const handleSubmitExam = () => {
    if (selectedExam) {
      ...
      // Cập nhật trạng thái exam đã hoàn thành
      const examIndex = mockExams.findIndex(e => e.id === selectedExam.id)
      if (examIndex !== -1) {
        mockExams[examIndex].completed = true
        mockExams[examIndex].score = result.score
        ...
        <div className="mx-auto px-4 py-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredExams.filter(exam => exam.type === selectedTab).map((exam) => (
              <div
                key={exam.id}
                className="bg-white border-2 border-gray-200 rounded-3xl p-6 hover:border-[#6E55FB] hover:shadow-2xl transition-all hover:scale-105 relative overflow-hidden group"
              >
                {/* Badge AI */}
                {exam.type === 'ai' && (
  ...
  // Màn hình 5: Xem lại kết quả (Review Result)
  if (currentScreen === 'reviewResult' && viewingResult && selectedExam) {
    const score = viewingResult.score
    const total = viewingResult.total
    const percentage = Math.round((score / (total * 2)) * 100)
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-8 mb-8">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <h2 className="text-2xl font-bold text-gray-900 mb-4">{selectedExam.name}</h2>
                <div className="space-y-2 text-lg text-gray-600">
                  <p className="flex items-center gap-2">
                    <svg className="w-5 h-5 text-[#6E55FB]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                    </svg>
                    <span className="font-semibold">Chương:</span> {selectedExam.chapter}
                  </p>
                  <p className="flex items-center gap-2">
                    <svg className="w-5 h-5 text-[#6E55FB]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                    <span className="font-semibold">Hoàn thành lúc:</span> {viewingResult.completedAt}
                  </p>
                </div>
              </div>

              <div className="bg-gradient-to-br from-[#6E55FB] to-purple-600 rounded-2xl p-6 text-white">
                <p className="text-xl mb-2">Kết quả</p>
                <p className="text-6xl font-bold mb-2">{score}/{total * 2}</p>
          ...
          <div className="space-y-6">
            {mockQuestions.map((question, index) => {
              const userAnswer = viewingResult.answers[index]
          ...
          <div className="mt-8 grid md:grid-cols-2 gap-6">
            ...
            <button
              onClick={() => handleRetakeExam(selectedExam)}
